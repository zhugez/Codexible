...
export function ActivityHeatmap({ data }: ActivityHeatmapProps) {
  ...
  const maxRequests = hasData ? Math.max(...data.map((d) => d.requests), 1) : 1;
    ...
    <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] p-4">
      ...
      <div className="flex items-end gap-1" style={{ height: 120 }}>
        {hasData ? (
          ...
          <div className="flex h-full w-full items-center justify-center rounded-lg border border-dashed border-[var(--border)] text-xs text-[var(--text-muted)]">
            No hourly data yet
          </div>
        )}
      </div>
